...
public class Stroke implements Serializable
{
    ...
    public double calculDistance(Stroke other) {
        double tmp = 0.0;
        for(int i=0; i< NB_POINTS; i++ ) {
            tmp += Math.sqrt(Math.pow(this.listePoint.get(i).x - other.listePoint.get(i).x, 2) +
                    Math.pow(this.listePoint.get(i).y - other.listePoint.get(i).y, 2));
        }
        return tmp / NB_POINTS;
    }
